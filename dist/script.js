...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (Jet);
...
const reg = () => {
  ...
  registrationForm.addEventListener('submit', function (event) {
    ...
    const registerUserToStrapi = async (email, password) => {
      try {
        const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post('http://localhost:1337/api/auth/local/register', {
          username: email,
          // Используем email пользователя как его имя пользователя
          email: email,
          password: password
        });
    ...
    registerUserToStrapi(email, password).then(userData => {
      if (userData) {
...
/* harmony import */ var _modules_reg__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./modules/reg */ "./src/js/modules/reg.js");
